...
import { cookieKeyUser, onResponseNoAccount } from "@repo/config-static";
import type { ISession, IUser } from "@repo/shared-db";
import { onUpdateCookieUser, onValidateCookieUser } from "../cookie/cookieUser";
...
export async function middlewareUser(ctx: Context<IMiddlewareUser>, next: Next) {
	try {
		...
		deleteCookie(ctx, cookieKeyUser);
		return ctx.json({ state: "bad User operation" });
	}
}
